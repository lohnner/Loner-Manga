...
function renderCollection() { const cards=allCards(); $('#stickerGrid').innerHTML=cards.map(card=>state.collection[card.id]?cardMarkup(card,state.collection[card.id]):`<article class="sticker locked ${card.className}"><div class="locked-symbol">?</div><strong>${card.name}</strong><span>${card.label}</span></article>`).join(''); const owned=cards.filter(c=>state.collection[c.id]); $('#uniqueCount').textContent=owned.length; $('#collectionValue').textContent=`${cards.reduce((sum,c)=>sum+(state.collection[c.id]||0)*c.value,0)} LM`; }
