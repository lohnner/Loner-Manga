...
let user, shop, inventory = [], boxes = [], collectionItems = [], listings = [], publicShops = [], busy = false, activeTab = "boxes", activeCollectionCondition = "lacrado";
...
let scoreSyncing=false;
async function reconcileCollectionScore(){if(!user||!shop||scoreSyncing)return;const points=collectionItems.reduce((sum,item)=>sum+(GAME_CONFIG.conditions[item.condition||"lacrado"]?.collectionPoints||0),0),distinct=collectionItems.length;if(shop.collectionPoints===points&&shop.collectionDistinctCount===distinct)return;scoreSyncing=true;try{await dbSdk.setDoc(dbSdk.doc(db,"shops",user.uid),{collectionPoints:points,collectionDistinctCount:distinct,updatedAt:dbSdk.serverTimestamp()},{merge:true})}catch(e){console.error(e)}finally{scoreSyncing=false}}
...
function startWatchers() {
  ...
  unsubscribers.push(dbSdk.onSnapshot(dbSdk.doc(db,"shops",user.uid), snap=>{shop=snap.exists()?snap.data():null; render();reconcileCollectionScore();}));
  ...
  watchCollection(["shops",user.uid,"collectionVolumes"],v=>{collectionItems=v;reconcileCollectionScore()});
...
function itemCard(item, area="inventory") {
  ...
  if(area==="inventory") {
    const alreadyOwned=collectionItems.some(owned=>owned.volumeId===item.volumeId&&(owned.condition||"lacrado")===item.condition);
    actions=`<button data-act="sellToNpc" data-id="${esc(item.copyId||item.id)}">Vender por ${c.saleValue} LM</button><button data-act="list" data-id="${esc(item.copyId||item.id)}">Colocar à venda</button>${alreadyOwned?'<button type="button" disabled>Já Tem</button>':`<button data-act="collect" data-id="${esc(item.copyId||item.id)}">Adicionar à coleção</button>`}`;
  }
...
function collectionPoints(){return shop?.collectionPoints??collectionItems.reduce((sum,item)=>sum+(GAME_CONFIG.conditions[item.condition||"lacrado"]?.collectionPoints||0),0)}
function stats() { document.querySelector("#shopName").textContent=shop?.shopName||"Minha Loja"; document.querySelector("#storeStats").innerHTML=[[`Saldo`,`${shop?.lm??0} LM`],[`Inventário`,`${shop?.inventoryCount??0}/${shop?.inventoryCapacity??15}`],[`Prateleira`,`${shop?.marketListingCount??0}/${shop?.marketCapacity??5}`],[`Coleção`,`${collectionPoints()} pontos`]].map(([a,b])=>`<span class="stat-pill"><small>${a}</small><strong>${b}</strong></span>`).join(""); }
function boxesPanel(){
  ...
  const images={paid:"assets/caixas/caixa-gratuita.png",premium:"assets/caixas/caixa-lacrada.png",free:"assets/caixas/caixa-classica.png"};
  const visual=box=>`<div class="box-visual"><img src="${images[box.id]||images.paid}" alt="${esc(box.label)}" loading="lazy"></div>`;
  return `<section class="store-panel"><div class="panel-heading"><div><h2>Caixas</h2><p>O estoque é renovado a cada hora cheia no horário de Brasília.</p></div></div><div class="box-options">${Object.values(GAME_CONFIG.boxes).map(box=>`<article class="box-card box-card-${box.id}">${visual(box)}<h3>${box.label}</h3><p>${descriptions[box.id]}</p><strong>${box.price?`${box.price} LM`:"GRÁTIS"}</strong><button data-act="buyBox" data-type="${box.id}" ${busy||shop.lm<box.price?"disabled":""}>Comprar caixa</button></article>`).join("")}</div><h2>Suas caixas</h2><div class="store-grid owned-box-grid">${boxes.length?boxes.map(box=>{const config=GAME_CONFIG.boxes[box.type]||{id:"paid",label:"Caixa"};return `<article class="box-card owned-box-card box-card-${config.id}"><div class="box-visual ${box.status==="opened"?"box-opening":""}"><img src="${images[config.id]||images.paid}" alt="${esc(config.label)}" loading="lazy"></div><h3>${esc(config.label)}</h3><p>${box.status==="purchased"?"Pronta para abrir":box.claimed?"Recompensas recebidas":`Recompensas pendentes: ${box.rewards?.length||0}`}</p>${box.status==="purchased"?`<button data-act="openBox" data-id="${box.id}">Abrir caixa</button>`:""}${box.status==="opened"&&!box.claimed?`<button data-act="claimBox" data-id="${box.id}">Receber recompensas</button>`:""}</article>`}).join(""):"<p class='empty-state'>Nenhuma caixa comprada.</p>"}</div></section>`;
}
function collectionPanel(){ const conditionItems=collectionItems.filter(item=>(item.condition||"lacrado")===activeCollectionCondition),owned=new Map(conditionItems.map(x=>[x.volumeId,x])),groups=Map.groupBy?Map.groupBy(catalogoVolumes,v=>v.mangaId):catalogoVolumes.reduce((m,v)=>(m.set(v.mangaId,[...(m.get(v.mangaId)||[]),v]),m),new Map()),tabs=Object.entries(GAME_CONFIG.conditions).map(([id,c])=>`<button data-act="collectionCondition" data-type="${id}" class="${id===activeCollectionCondition?"active":""}">${c.label} · ${c.collectionPoints} pts</button>`).join(""),sortedGroups=[...groups].sort(([,a],[,b])=>String(a[0]?.mangaTitle||"").localeCompare(String(b[0]?.mangaTitle||""),"pt-BR",{sensitivity:"base"})); return `<section class="store-panel"><h2>Coleção — ${collectionPoints()} pontos</h2><p>Você pode guardar uma cópia de cada volume em cada condição.</p><div class="collection-condition-tabs">${tabs}</div><input class="store-search" id="collectionSearch" type="search" placeholder="Buscar mangá na coleção" autocomplete="off"><div class="store-grid" id="collectionMangaGrid">${sortedGroups.map(([id,vols])=>{const have=vols.filter(v=>owned.has(v.volumeId)).length,cover=vols.find(v=>v.volumeNumber===1)||vols[0];return `<article class="item-card collection-group ${activeCollectionCondition}" data-act="openCollection" data-id="${id}"><img class="collection-cover ${have?"":"locked"}" src="${coverUrl(cover.cover)}" alt="Capa de ${esc(cover.mangaTitle)}"><div class="item-body"><span class="condition condition-${activeCollectionCondition}">${condition(activeCollectionCondition).label}</span><h3>${esc(cover.mangaTitle)}</h3><strong>${have} de ${vols.length} volumes distintos</strong></div></article>`}).join("")}</div></section>`; }
...
function renderHourlyBoxStock(){
  ...
  document.querySelectorAll('.box-options [data-act="buyBox"]').forEach(button=>{
    ...
    if(box.id==="free"&&remaining>0&&!button.closest(".box-card")?.querySelector(".free-box-alert"))button.closest(".box-card")?.querySelector(".box-visual")?.insertAdjacentHTML("afterend",'<strong class="free-box-alert">Caixa Gratuita Disponível</strong>');
...
async function handleClick(e){
  ...
  if(act==="collectionCondition"){activeCollectionCondition=type;render();}
  if(act==="list"){
  ...
  if(act==="openCollection"){
    const vols=catalogoVolumes.filter(v=>v.mangaId===id).sort((a,b)=>a.volumeNumber-b.volumeNumber),owned=new Map(collectionItems.filter(x=>(x.condition||"lacrado")===activeCollectionCondition).map(x=>[x.volumeId,x])),c=condition(activeCollectionCondition);
    document.querySelector("#dialogContent").innerHTML=`<h2>${esc(vols[0]?.mangaTitle)} — ${c.label}</h2><div class="store-grid">${vols.map(v=>{const o=owned.get(v.volumeId);return `<article class="item-card ${activeCollectionCondition}"><img class="collection-cover ${o?"":"locked"}" src="${coverUrl(v.cover)}" alt="Capa volume ${v.volumeNumber}"><div class="item-body"><span class="condition condition-${activeCollectionCondition}">${c.label}</span><h3>Volume ${v.volumeNumber}</h3><strong>Possuídos: ${o?.quantity||0}</strong>${o?`<button data-act="removeCollection" data-id="${esc(o.id)}">Retirar uma cópia</button>`:""}</div></article>`}).join("")}</div>`;dialog.showModal();
  }
  if(act==="removeCollection")await action("removeFromCollection",{collectionId:id});
}
document.addEventListener("click",handleClick);document.querySelector("[data-close-dialog]").onclick=()=>dialog.close();document.querySelector(".store-tabs").onclick=e=>{const b=e.target.closest("[data-tab]");if(b){activeTab=b.dataset.tab;render()}};document.addEventListener("input",e=>{if(e.target.id==="shopSearch")document.querySelector("#shopList").innerHTML=shopRows(e.target.value);if(e.target.id==="collectionSearch"){const q=e.target.value.trim().toLocaleLowerCase("pt-BR").normalize("NFD").replace(/[\u0300-\u036f]/g,"");document.querySelectorAll("#collectionMangaGrid .collection-group").forEach(card=>{const text=card.textContent.toLocaleLowerCase("pt-BR").normalize("NFD").replace(/[\u0300-\u036f]/g,"");card.hidden=Boolean(q)&&!text.includes(q)})}});document.addEventListener("submit",async e=>{if(e.target.id!=="createShopForm")return;e.preventDefault();const shopName=document.querySelector("#newShopName").value.trim();if(shopName.length<3||shopName.length>24)return message("O nome deve ter entre 3 e 24 caracteres.","error");await action("createShop",{shopName})});
